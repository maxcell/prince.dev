...
As I slowly migrate my blog to use some new tools, I wanted to make sure that 
I kept a good record as far as what tools I have been using and how I've been moving
around with them. One of the most important things for me as I switch to [MDX][mdx] has been
maintaining the readability of my code blocks. I won't be talking about how to install MDX
but the maintainers have a [great set of docs about it for Gatsby][mdx-gatsby] (which is what I use)!

## Adding a `CodeBlock` component to MDX

To start off, we need to make sure that out layout file has `MDXProvider` and we can add in
custom components that match to their [shortcodes][mdx-shortcodes]. In my case, 
I needed to declare the component for `code`. I went to my `src/pages/layout.js` and added:
...
prop called `language` to `<Highlight>`.
2. We will pass in the `children` prop as a pro named `code` to `<Highlight>`. Then we'll have 
...
a bit personal so I will show you 1) how to use other preset themes and 2) how to reuse the ones you had
...
[here][prism-react-renderer-themes]. By default it uses the `duotoneDark` theme. To add the theme,
you want to import the specific one and then pass it as the `theme` prop for `Highlight`:
...
to tell the `theme` is `undefined`.

So make sure in you have your css file imported:

```js
// gatsby-browser.js
 
// This is where source path to my CSS theme
