...
While going through [the Rust + wasm book][rust-wasm-book], I wanted to archive some of the knowledge
it gave me. Particularly because this is the first time I wrote any Rust + WebAssembly (Wasm) code,
let alone used it in the browser!
...
- Pull down the cargo template
- Look into the `src/lib.rs`
- We can add connections to the methods afford to use through the `extern`
- We can add new functions and then connect them with the `wasm_bindgen`

- We can also connect directly through with the js_namespace `#[wasm_bindgen(js_namespace = console)]`